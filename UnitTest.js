...
function loadData() {
    ...
    fetch('http://localhost:8081/viewUnitTest?email=' + email)
...
for (i = 0; i < rowLength; i++){
   ...
   for(var j = 1; j < cellLength; j++){
    ...
    unitTestList.push(unitTest);
}
    
    let email = localStorage.getItem("teacherEmail");
    const Url = 'http://localhost:8081/saveUnitTest';
    const saveUnitTest = {
        email: email,
    ...
    const success = function (reponse) {
        ...
        window.location = "http://127.0.0.1:5500/UnitTest.html"
